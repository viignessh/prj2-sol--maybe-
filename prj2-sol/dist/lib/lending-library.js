...
export class LendingLibrary {
    ...
    async addBook(req) {
        ...
        const validation = Lib.validate('addBook', req);
        if (!validation.isOk) {
            return validation; // Cast to the expected type
        }
    ...
    async findBooks(req) {
        // Validate the incoming request
        const validation = Lib.validate('findBooks', req);
        if (!validation.isOk) {
        ...
        const findRequest = { search, index, count };
        ...
        return result.isOk
            ? Errors.okResult(result.val) // Cast to the expected type if needed
    ...
    async checkoutBook(req) {
        // Validate the incoming request
        const validation = Lib.validate('checkoutBook', req);
        if (!validation.isOk) {
    ...
    async returnBook(req) {
        ...
        const validation = Lib.validate('returnBook', req);
        if (!validation.isOk) {
